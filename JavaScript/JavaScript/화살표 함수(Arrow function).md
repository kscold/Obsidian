...
- 위의 경우 [[addEventListener()]]가 [[콜백 함수(Callback Function)]]를 호출할 때 [[this]]를 이벤트 대상([[이벤트(event)]] target)인 button [[객체(Object)]]로 [[바인딩(binding)]]한다.
- 그러나 아래의 화살표 함수로 선언한 경우 [[this]]가 함수가 **정의된 위치의 외부 [[this]]** ([[렉시컬 환경(Lexical Environment)]]상의 [[this]])를 가리키므로 button [[객체(Object)]]가 아니다.
- 브라우저 전역에서 정의했다면 [[this]]는 window를 가리키고, [[strict mode]] 또는 ES module 환경에서는 undefined가 되어 `this.textContent` 접근 시 에러가 발생한다.

```js
button.addEventListener('click', () => {
	console.log(this.textContent); // 의도와 다르게 동작 / 에러
});
```

- 따라서 위의 코드를 [[화살표 함수(Arrow function)]]로 버튼 [[객체(Object)]]를 동작하게 하기 위해서는 [[이벤트(event)]] [[객체(Object)]]를 받아 e.target.textContent로 접근할 수 있다.
...
- 일반 [[함수(Function)]]는 **호출되는 방식**에 따라 [[this]]가 동적으로 결정된다(메서드 호출 시 메서드를 소유한 [[객체(Object)]], 생성자 호출 시 새로 만들어진 [[인스턴스(Instance)]], 일반 호출 시 window/undefined).
- 반면 화살표 함수는 자신만의 [[this]]를 가지지 않고, 함수가 **정의될 때의 외부 [[렉시컬 환경(Lexical Environment)]]**의 [[this]]를 그대로 사용한다(이를 lexical this라고 한다).
