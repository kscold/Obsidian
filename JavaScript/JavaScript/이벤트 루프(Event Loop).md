...
- 즉, 백그라운드는 브라우저마다 자바스크립트가 아닌 C++ 등 네이티브 코드나 운영체제 API로 구현되어 있다(Node.js의 경우 libuv).
...
- [[테스크 큐(Task Queue)]]와는 별도로 마이크로태스크 큐가 존재한다.
- [[Promise]]의 `.then`/`.catch`/`.finally` [[콜백 함수(Callback Function)]], `queueMicrotask()`, `MutationObserver` 콜백 등이 여기에 쌓인다.
- 이벤트 루프는 [[호출 스택(Call Stack)]]이 비었을 때 **마이크로태스크 큐를 먼저 모두 비우고**, 그 다음에 [[테스크 큐(Task Queue)]]에서 하나를 꺼내 실행한다.
- 즉, [[Promise]] 후속 작업은 [[setTimeout()]]/`setInterval`/I/O 같은 매크로태스크보다 항상 먼저 실행된다.
