...
- 복잡하지 않은 일반적인 [[쿼리(Query)]]를 실행할 때에는 [[리파지터리(Repository)]]를 사용하는게 편리하다.
- 조금 더 복잡한 [[쿼리(Query)]]를 실행해야 하거나 다이나믹하게 궈키를 만들어 가야 할 경우 쿼리빌더(QueryBuilder)를 사용해야 한다.
...
- 따라서 결론은 [[SQL(Structured Query Language)]]을 명확히 이해하고 DB를 다루는 기본적인 능력을 기르도록 하자.
- 쿼리를 손수 작성하는 방법도 있지만, [[SQL(Structured Query Language)]] [[쿼리(Query)]]에 대한 이해를 바탕으로, QueryBuilder을 사용하도록 하자.
...
- 하나의 데이터만 얻으려면 [[QueryBuilder.getOne()]] [[메서드(Method)]]를 사용한다.
