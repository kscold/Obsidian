...
- [[타입스크립트(TypeScript)]]로 짜여진 코드를 자바스크립트로 변환해 줄 트랜스파일러(컴파일러)가 필요하다.


## tsconfig.json 생성

- `tsconfig.json`의 기본 형태는 다음 커맨드를 통해 생성할 수 있다.

```bash
npx tsc --init
```

- 해당 커맨드로 생성되는 기본 형태의 `tsconfig.json` 및 옵션들은 다음과 같다.


## tsconfig.json의 컴파일 옵션 정리

```json
{  
	"compilerOptions": {    
	/* https://aka.ms/tsconfig.json 를 방문하면 해당 파일에 대한 더 많은 정보를 얻을 수 있습니다.*/     // 옵션은 아래와 같은 형식으로 구성되어 있습니다.    
	// "모듈 키": 모듈 값                        
	/* 설명: 사용가능 옵션 (설명이 "~ 여부"인 경우 'true', 'false') */     
