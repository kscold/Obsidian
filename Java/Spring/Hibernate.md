...
- Hibernate는 자바 [[객체(Object)]]를 통해 [[데이터베이스(DataBase)]]가 Oracle, MySql, MSSQL 등에 상관없이 다룰 수 있도록 하는 추상화를 목표로 한다.
- Mybatis에 익숙한 사람들이라면, Mybatis 구조가 들어가는 자리에 Hibernate가 들어오는 구조라고 이해하면 된다.
