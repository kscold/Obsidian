- 영속성 컨텍스트(Persistence Context)는 **JPA가 엔티티 객체를 관리하는 논리적인 공간**이다.
- `EntityManager`가 영속성 컨텍스트와 1:1로 연결되어 엔티티의 생명주기를 관리한다.
- Spring Boot + Spring Data JPA 환경에서는 `@Transactional` 범위 내에서 자동으로 동작한다.
...
| 영속(Managed) | 컨텍스트가 관리하는 상태 | 트랜잭션 커밋 시 |
...
```java
Post post1 = postRepository.findById(1L).get();  // DB 조회 → 캐시 저장
...
- 트랜잭션 커밋 시점에 **스냅샷(최초 조회 시 상태)**과 현재 객체를 비교해서 변경된 필드만 UPDATE한다.
- `save()`를 명시적으로 호출하지 않아도 필드 값을 바꾸면 DB에 반영된다.
...
- SQL을 즉시 DB에 보내지 않고 **쓰기 지연 저장소**에 모아두었다가 트랜잭션 커밋 직전에 한꺼번에 실행한다.
- 여러 INSERT/UPDATE를 모아서 배치로 실행하면 DB I/O가 줄어든다.
...
- 장점: 뷰/컨트롤러에서도 지연 로딩 가능.
- **단점: DB 커넥션을 요청 전체 시간 동안 점유 → 트래픽 많으면 커넥션 부족**.
- 실무 권장: `spring.jpa.open-in-view=false` + 서비스 계층에서 DTO로 변환.
...
- 준영속 상태에서 지연 로딩을 시도하면 `LazyInitializationException` 발생.
