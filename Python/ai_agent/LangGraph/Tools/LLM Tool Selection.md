...
도구 선택 정확도는 단지 description 품질만의 문제가 아니다. 모델은 **이번 호출에 실제로 노출된 도구 집합** 안에서만 선택할 수 있다. 사용자 권한, 현재 workflow 단계, feature flag, 비용 예산, 데이터 범위에 따라 도구 후보를 먼저 줄이는 동적 노출 정책이 필요하다.

1. Capability Catalog에서 현재 요청에 가능한 도구 후보를 찾는다.
2. 권한·위험도·테넌트 범위를 코드 정책으로 걸러 낸다.
3. 남은 작은 도구 집합만 모델에 바인딩한다.
4. ToolNode 또는 wrapper가 실행 전 인자와 권한을 다시 검증한다.

이 순서는 모델에게 더 많은 도구를 보여 주는 것보다 선택 오류와 토큰 사용을 줄인다. 단, 도구를 숨기는 정책도 감사 가능해야 하며, 모델이 보지 못한 도구를 호출할 수 없다는 사실을 UI와 fallback 정책에 반영한다.

관련: [[Capability Catalog]], [[Tool Execution Policy]], [[Agent Middleware]]
